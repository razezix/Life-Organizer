...
export default function Goals() {
  ...
  const load = async () => {
    setLoading(true)
    const data = await apiCall(() => goalsApi.list())
    if (data) setGoals(data.items)
    setLoading(false)
  }
  ...
  const create = async (e: React.FormEvent) => {
    ...
    await apiCall(() => goalsApi.create({ title }))
    setTitle('')
    load()
  }

  const updateProgress = async (id: number, progress: number) => {
    await apiCall(() => goalsApi.update(id, { progress }))
    load()
  }

  const addMilestone = async (goalId: number) => {
    const txt = milestoneInputs[goalId]?.trim()
    if (!txt) return
    await apiCall(() => goalsApi.addMilestone(goalId, txt))
    setMilestoneInputs((prev) => ({ ...prev, [goalId]: '' }))
    load()
  }

  const toggleMilestone = async (goalId: number, m: Milestone) => {
    await apiCall(() => goalsApi.updateMilestone(goalId, m.id, { completed: !m.completed }))
    load()
  }

  const remove = async (id: number) => {
    await apiCall(() => goalsApi.remove(id))
    load()
  }
  ...
  return (
    <Layout>
      <div className="max-w-2xl">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">{t('goals')}</h1>

        <form onSubmit={create} className="bg-white rounded-xl shadow-sm p-5 mb-6 flex gap-3">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t('newGoal')}
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {goals.map((goal) => (
              <div key={goal.id} className="bg-white rounded-xl shadow-sm p-5">
                ...
                <div className="flex gap-2">
                  <input value={milestoneInputs[goal.id] || ''}
            ...
            {goals.length === 0 && (
              <p className="text-gray-400 text-sm text-center py-8">{t('noGoalsYet')}</p>
            )}
          </div>
        )}
      </div>
